...
class Keys:
    ...
    @staticmethod
    def check_function(function_name):
        check_list = [x for x in dir(Keys) if not x.startswith("__")]
        return function_name in check_list
...
class SettingsFileManipulator:
    ...
    def __init__(self):
        ...
        self.settings_filename_with_path = functions.unixify_path_join(
            data.settings_directory, self.settings_filename
        )
    ...
    def save_settings(self, main_window_side, theme, context_menu_functions=None):
    ...
    def add_session(self, session_name, session_group, main, upper, lower):
...
def parse_settings_file(settings_filename_with_path):
    ...
    # Import the init file as a python module
    init_module = runpy.run_path(
        settings_filename_with_path, init_globals={"themes": themes, "data": data}
    )
    return {
